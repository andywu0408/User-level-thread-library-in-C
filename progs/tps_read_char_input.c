...
void *thread2(__attribute__((unused)) void *arg)
{
    ...
    tps_create();
    tps_write(0, sizeof(char), &msg1);

    /* Read from TPS and make sure it contains the message */
    memset(buffer, 0, sizeof(char));
    tps_read(0, sizeof(char), buffer);
    assert(!memcmp(&msg1, buffer, sizeof(char)));
    printf("thread2: read char OK!\n");
...
void *thread1(__attribute__((unused)) void *arg)
{
    ...
    tps_read(0, sizeof(char), buffer);
    assert(!memcmp(&msg1, buffer, sizeof(char)));
    printf("thread1: read char OK!\n");

    /* Modify TPS to cause a copy on write */
    buffer[0] = 'h';
    tps_write(0, sizeof(char), buffer);
