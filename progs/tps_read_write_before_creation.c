...
void *thread2(__attribute__((unused)) void *arg)
{
	...
	/* Check if writing before creation raises error */
	assert(tps_write(0, TPS_SIZE, msg1) == -1);
	printf("thread2: passed check for writing before creation\n");
...
void *thread1(__attribute__((unused)) void *arg)
{
	...
	/* Check if cloning a tps that is not created raises error */
	assert(tps_clone(tid) == -1);
	printf("thread1: passed check for cloning a tps that is not created\n");
